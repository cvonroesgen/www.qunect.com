...
{   
    ...
    dbid = RegExp.$1;
    var clist ="0";
    var NameValues = qdb.parseQueryString();
	if(NameValues.dbid)
	  {
	   dbid = NameValues.dbid;
	   }
   if(NameValues.rid)
	  {
	   rid = NameValues.rid;
   ...
   if(NameValues.clist)
	  {
	   clist = NameValues.clist;
	   }
   if(NameValues.sfid)
	  {
	   sfid = NameValues.sfid;
	   }
   if(NameValues.pagename)
	  {
	   pagename = NameValues.pagename;
	   }
	if(NameValues.apptoken)
		{
		qdb.SetAppToken(NameValues.apptoken);
		}
   if(rid == "" && qid == "")
        {
   ...
   if(clist =="0")
      {
       ...
       ridFid = schema.find("field[field_type = 'recordid']").attr("id");
       var fields = schema.find("field");
       clist="";
       fields.each(function(i, e)
          {
           var thisField = $(e);
           var txtfid = thisField.attr("id");
           clist += txtfid+ ".";
           var fieldLabel = thisField.find("label").text();
    ...
    if(qid != "")
        {
        jTable = qdb.DoQuery(dbid, qid, clist, "");
        }
    else
        {
        jTable = qdb.DoQuery(dbid, "{'"+ ridFid +"'.EX.'"+ rid +"'}", clist, "");
        }
   if(clist !="0")
        {
        var fields = jTable.find("field");
        fields.each(function(i, e)
          {
           var thisField = $(e);
           var txtfid = thisField.attr("id");
    ...
    var variables = jTable.find("var");
    variables.each(function(i, e)
        {
        var qvar = $(e);
        var varname = qvar.attr("name");
        if(varname.toLowerCase() == 'length')
          {
          varname = 'qdbEF_length';
          }
        vars[varname] = qvar.text();        
        vars[varname.toLowerCase()] = qvar.text();
        }
	);
    var templateHTML = window.document.body.innerHTML;
    window.document.body.innerHTML = ""
    var records = jTable.find("record");
    var pageCounter = 0;
    
    records.each(function(i, e)
        {
        record = $(e);
        fillRecordArrays(jTable, record.find("f"));
...
function fillRecordArrays(jTable, fields)
{
	fields.each(function(i, e)
		{
		var thisField = $(e);
		var txtfid = thisField.attr("id");
		var data = getFormattedField(jTable, thisField);
...
function getFormattedField(jTable, thisField)
{
	var data = thisField.text();
	return formatText(jTable, thisField, data);
}

function formatText(jTable, thisField, data)
{
		var txtfid = thisField.attr("id");
	
       var field_type = jTable.find("field[id="+txtfid+"]").attr("field_type");
       var base_type = jTable.find("field[id="+txtfid+"]").attr("base_type");
       
       f[txtfid] = data;
       if(field_type == "timestamp" || field_type == "date")
          {
           data = qdb.format(data, "date");           
           }
       else if(field_type == "timeofday")
           {
           data = qdb.format(data, "timeofday");            
           }
       else if(field_type == "file")
           {
            var url = thisField.find("url").text();
            if(url)
              {
              data = "<img src='" + url + "'>";
               }
            ...
            {
            var target_dbid = jTable.find("field[id="+txtfid+"]").find("target_dbid").text();
            var target_fid = jTable.find("field[id="+txtfid+"]").find("target_fid").text();
            var url = "API_GenResultsTable&query={'"+target_fid+"'.EX.'" + data + "'}&options=nvw.ned.phd.nfg";
            data = "Code to be written: " + url;
            }
       else if(field_type == "text")
           {
            var allowHTMLnode = jTable.find("field[id="+txtfid+"]").find("allowHTML").text();
			if(allowHTMLnode == "1")
				{
				data = thisField.html();
				}
       ...
       else if(base_type == "float")
            {
             ...
             var commas = jTable.find("field[id="+txtfid+"]").find("comma_start").text();
             if(commas)
                {
                commas=parseInt(commas);
                }
             else
				 {
				 commas = 0;
				 }
             var decimal_places = jTable.find("field[id="+txtfid+"]").find("decimal_places").text();
             if(decimal_places)
                {
             ...
             if(field_type == "currency")
                 {
                  currency_symbol = jTable.find("field[id="+txtfid+"]").find("currency_symbol").text();
                  currency_format = jTable.find("field[id="+txtfid+"]").find("currency_format").text();
                  }
            ...
            {
            var luserID = jTable.find("luser[id='"+data+"']");
            if(luserID)
                {                        
                data = luserID.text();
                }
...
function table(dbid, query, clist, slist, options)
{
var jData = qdb.DoQuery(dbid, query, clist, slist, options);
var jRecords = jData.find("record");
var jFields = jData.find("field");
var jHTML = $("<table id=dbid-"+dbid+"></table>");
var jHead = $("<thead><tr></tr></thead>");
jHTML.append(jHead);
var jHeadRow = jHead.find("tr");
var fieldProps = {};
var totals = {};
jFields.each(
	function(i, e)
		{
		var jField = $(e);
		var fid = jField.attr("id");
		fieldProps[fid] = {};
		var does_total = jField.find("does_total").text();
		fieldProps[fid].does_total = does_total;
		if(does_total == "1")
			{
			totals[jField.attr("id")] = 0;
			}
		else
			{
			totals[jField.attr("id")] = "";
			}
		var jCell = $("<th class=fid" + fid + "></th>").text(jField.find("label").text());
		jHeadRow.append(jCell);
		}
	);
jRecords.each(
	function(i,e)
		...
		var jRecord = $(e);
		var jRow = $("<tr>");
		var jColumns = jRecord.find("f");
		jColumns.each(
			function(i, e)
			{
				var jColumn = $(e);
				var fid = jColumn.attr("id");
				var jField = jData.find("field[id="+jColumn.attr("id")+"]");
				var strContent = getFormattedField(jData, jColumn);
				jRow.append($("<td class=fid" + fid + ">").text(strContent));
				if(fieldProps[fid].does_total == "1")
				{
					totals[fid] += parseFloat(strContent.replace(/[^\d\-\.]/g, ""));
				}
			}
		);
		jHTML.append(jRow);
...
var jFoot = $("<tfoot><tr></tr></tfoot>");
jHTML.append(jFoot);
var jFootRow = jFoot.find("tr");
jFields.each(
	function(i, e)
		{
		var jField = $(e);
		...
		if(fieldProps[fid].does_total == "1")
			{
			var total = formatText(jData, jField, totals[fid].toString());
			}
		else
			{
			var total = "";	
			}
		var jCell = $("<td class=fid" + fid + "></td>").text(total);
		jFootRow.append(jCell);
		}
	);


return jHTML.prop('outerHTML');
}
...
function snapShotEF(event)
  ...
  if (event)
    ...
    if(sfid)
        ...
        var fileAttachmentFields = jTable.find("field[field_type=file]");
        fileAttachmentFields.each(function(i, e)
            {
			var field = $(e);
        ...
        $("#oksfid").click(saveSFID);
        $("#cancelsfid").click(hideDiv);
        }
  }
...
var efButtons = schema.find("field[field_type=url][mode=virtual]");
...
efButtons.each(function(i, e)
    {
	var button = $(e);
    var formula = button.find("formula").text();
    if(formula)
        {
...
function QuickBaseClient( qdbServer )
{
    ...
    */
    ...
    this.addParameter = function(xmlQDBRequest, Name, Value)
        ...
        //xmlQDBRequest.payload = concatenate(Uint8Array, xmlQDBRequest.payload, str2ab( "--" + multiPartBoundary + "\r\n" + "Content-Disposition: form-data; name=\"" + Name + "\"\r\n\r\n" + Value + "\r\n"));
        }
    ...
    this.addFieldParameter = function ( xmlQDBRequest, fid, Value )
        ...
        if(typeof(fid) == "string")
            ...
            {
            xmlQDBRequest.payload += "--" + multiPartBoundary + "\r\n" + "Content-Disposition: form-data; name=\"_fid_" + fid.fid + "\"; filename=\""+ fid.fileName +"\"\r\n\r\n" + Value + "\r\n";
            //xmlQDBRequest.payload = concatenate(Uint8Array, xmlQDBRequest.payload, str2ab("--" + multiPartBoundary + "\r\n" + "Content-Disposition: form-data; name=\"_fid_" + fid.fid + "\"; filename=\""+ fid.fileName +"\"\r\n\r\n"), Value, str2ab("\r\n"));
            }
        }
